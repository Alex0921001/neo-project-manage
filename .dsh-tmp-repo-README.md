...
- **Agent 直连**：Agent 和面板共用同一份主��配置，对话里说一句「连一下 xxx 看看状态」，智能体就去执行远程命令。
...
不想装全家��时，可单独安装任意插件（npm 已发布，直接用包名）：
...
A: 先确认插件装进了 `web` profile（命令里�� `--profile web`），再用 `dsh --profile web --dump-config` 确认插件配置层已挂载；还不行就看上文「安装排障」。注意页面刷新不够，要重启 `dsh web` 进程。
...
A: 只要皮��就装 `@linxin666/dsh-client-ui-skin-center`；只装某一个插件就用「单独安装某个插件」里的包名，两者都走 npm 安装。
...
- SSH 密码与 passphrase 口令以明文保存在 `~/.dsh/dsh-ssh.json`（权限 0600）；断线重连可能重放非幂等命令，远程输出原样返回、不脱敏，安全模��见 [dsh-ssh README](packages/dsh-ssh/README.zh.md)。
...
| 包 | 来源 | 版权 |
| --- | --- | --- |
| dsh-task-board / dsh-git-graph / dsh-aionui-panel / dsh-pet / dsh-remote-web-ui / dsh-web-ui-settings / dsh-liangshen / dsh-skins / dsh-web-ui-all / skins | 作者 zhu1090093659 个人开发 | Apache-2.0（zhu1090093659） |
