...
onMounted(async () => {
  ...
  // Ctrl+F 全局搜索（插件 iframe 内，若宿主拦截该快捷键则事件不达，自然降级为不拦截）
  window.addEventListener("keydown", onGlobalKeydown);
  // 搜索/消息跳转事件（跨项目时切到目标项目；同项目由项目页自行处理）
...
function onGlobalKeydown(e) {
  ...
  if (e.ctrlKey && (e.key === "f" || e.key === "F")) {
    // 项目详情页内交给项目内搜索（index.vue 自己处理），避免双弹窗
