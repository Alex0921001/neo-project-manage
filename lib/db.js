...
-- 文件夹（文件系统重构）：多层嵌套，parent_id 自引用（NULL=根）
...
-- 成员表（成员管理）：全局成员名录，供人员下拉公共组件 / 历史人名补录
...
-- 审计日志表（审计追踪）：所有写操作留痕，按 project_id 归属（可空：全局成员/项目集操作）
...
export function createDb(dataDir) {
 ...
 // requirements 表 + requirement_plans 关联表（需求管理）：幂等建表
  migrateRequirementsTables(db);
 // file_folders 表 + files.folder_id 列（文件系统重构）：幂等建表/补列
 ...
 // verifications 验证清单表：幂等
  migrateVerificationsTable(db);
 // deleted_batch_keys 消息删除抑制表：幂等
 ...
 * 与后续  可能共用 v9 语义，先占 v9 版本号， 届时用 v10。
 *
 * @param {Database} db
 */
 ...
 *
 * 任务和方案双向关联：多对多挂载（task_id ↔ plan_id，双外键 CASCADE）。
...
/**
 * Schema 迁移到 v11（幂等）：消息中心 + 全文检索（一个函数建全部新表）
 *
 * 消息中心：
 ...
 *
 * 项目级风险规则配置：JSON 字符串，覆盖 7 条风险识别规则的开关/阈值/等级。
...
/**
 * 补齐 requirements / requirement_plans 表（需求管理，幂等）
...
export function migrateVerificationsTable(db) {
 ...
 // 验证项 Agent 化——kind（执行方式 human/agent/assertion）、instruction（agent 验证指令）、
  // evidence（证据 JSON：runner/runAt/summary/detail）。幂等补列，不 bump SCHEMA_VERSION
  // （与 plan_ids 同策略：存量列/索引语义不变，kind 默认 human 对存量数据无感）
  const icols = db.prepare("PRAGMA table_info(verification_items)").all().map((c) => c.name);
  if (icols.length) {
    if (!icols.includes("kind")) db.exec("ALTER TABLE verification_items ADD COLUMN kind TEXT NOT NULL DEFAULT 'human'");
    if (!icols.includes("instruction")) db.exec("ALTER TABLE verification_items ADD COLUMN instruction TEXT");
    if (!icols.includes("evidence")) db.exec("ALTER TABLE verification_items ADD COLUMN evidence TEXT");
  }
