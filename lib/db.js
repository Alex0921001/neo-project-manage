...
// better-sqlite3 vendored to ./vendor/better-sqlite3/ (avoid bare specifier —
// Hana dev slot excludes node_modules from plugin sync, so the host's
// better-sqlite3 in artifacts/server/.../node_modules is unreachable from
// the plugin import chain).
import Database from "./vendor/better-sqlite3/lib/index.js";

// native binding 绝对路径：绕过 better-sqlite3 内部对 `bindings` 包的依赖。
// 否则 vendor/.../node_modules/bindings/ 会被 Hana cp filter 过滤，require 失败。
...
export function createDb(dataDir) {
  ...
  const db = new Database(dbPath, { nativeBinding: NATIVE_BINDING });
