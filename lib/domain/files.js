...
export function createFilesModule(ctx) {
 ...
 // 摘要上限（字）：超出截断，避免登记冗余（文件资产化）
  ...
  /**
 * 读取文件元信息（size/ext），路径失效或非文件时兜底 null，不抛错（文件资产化）
  ...
  /**
 * 归一化 digest：空值兜底 null，超长截断到 DIGEST_MAX_LEN（文件资产化）
 ...
 * 移动文件到文件夹（folderId NULL=根目录）
 ...
 // 补审：先取这些文件夹下的文件明细（磁盘文件不动），删除后逐条审计（与 deleteFile 同款字段）
