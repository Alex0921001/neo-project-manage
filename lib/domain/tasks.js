...
export function createTasksModule(ctx) {
 ...
 // 任务反向展示关联方案（含方案标题）
    ...
    `).all(taskId);
 // 一致性：任务关联文件（含名称/类型，供工具输出；文件已删时该引用随 join 消失）
 ...
 // 关联方案（白名单校验：必须属于本项目，防跨项目/脏数据；任务只能关联已采纳的方案）
 ...
 //  规则：任务完成前置校验，该任务下所有便利贴必须已确认（含子任务规则先放行）
 ...
 // 完成任务时校验后代无未完成，防「父完成 + 子未完成」矛盾树（与便利贴校验并列）
 ...
 // done 状态无实际变化时不重复写（幂等）：避免已完成任务再传 done=true 刷新 done_at
      if (!!data.done !== !!cur.done) {
 ...
 // 关联方案全量替换（白名单校验：必须属于本项目；任务只能关联已采纳的方案）
 ...
 * 批量更新任务（逐条独立语义）
 ...
 // 收集本任务及全部后代，先清 plans.task_id 悬空引用（plans 无外键级联，须应用层清理）
 ...
 // 补审：批量重排，targetId 置空，newValue 记录完整顺序
 ...
 // 补审：移动前旧位置（parent_task_id + index_num）留作审计 oldValue
