 ...
 * 汇总项目全部结构化数据，生成固定模板的状态总结）
 ...
 // —— 待确认批注（kind 随批注类型，）——
    ...
    return {
      ...
      files,
      nextSteps,
 // 概览面板：结构化任务列表（带 id + 完整名）供 popover 快速定位
      pendingTaskItems,
      noDateTaskItems,
  ...
  /**
 * 解析周报时间范围）：thisWeek / lastWeek / last7days / custom
 ...
 * 与  localToday 的本地时区语义保持一致（避免 UTC 跨日偏差）
   * @param {*} s
   * @returns {number}
   */
