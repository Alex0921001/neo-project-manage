...
import { anchorQuoteInHtml } from "../quote-anchor.js";
...
export function createCommentsModule(ctx) {
  ...
  const htmlToPlain = (...a) => ctx.htmlToPlain(...a);
 // ===== 统一评论（comments 表：需求/方案共用）=====
  ...
  function commentRowToObject(r) {
    return {
      ...
      quoteAnchor: r.quote_anchor || null,
    };
  }
 ...
 // 自动补锚：Agent/MCP 只传 quoteText 无 quoteAnchor 时，后台在正文定位并包裹
    // 标注 span，使评论可在前端点击定位。划词场景前端已传 quoteAnchor 且自行包裹正文，跳过。
    // 补锚失败（找不到文字/解析异常）降级为纯文字引用，不影响评论创建。
    let anchoredHtml = null;
    if (comment.quoteText && !comment.quoteAnchor && bodyHtml != null) {
      try {
        const hit = anchorQuoteInHtml(bodyHtml, comment.quoteText, comment.id);
        if (hit) {
          comment.quoteAnchor = JSON.stringify(hit.anchor);
          anchoredHtml = hit.html;
        }
      } catch { /* 补锚异常按降级处理 */ }
    }
    db.prepare(`
    ...
    // 正文写入标注（复用划词引用写入口：绕状态冻结/版本快照，审计留「评论引用标注」）
    if (anchoredHtml != null) applyQuoteAnchor(projectId, comment.id, anchoredHtml);
 ...
 // 同步摘除旧表同 id 行（e05fbe69）：旧表保留只读，但已删行必须同步删除，
    // 否则 comments 幂等迁移按 id 判重（NOT EXISTS）会在下次库初始化时把已删评论搬回来（删除复活 bug）
    if (row.target_type === "plan") {
      db.prepare("DELETE FROM plan_comments WHERE id = ?").run(commentId);
    }
    // 删除审计带内容快照，可追溯删了什么
  ...
  /**
 * 评论引用标注写入（划词引用）：前端包裹好高亮 span 的新 HTML 直接写入正文。
  ...
  function applyQuoteAnchor(projectId, commentId, content, cleanup = null) {
    ...
    const row = db.prepare("SELECT * FROM comments WHERE id = ? AND project_id = ?").get(commentId, projectId);
 // 清理模式：评论已删除时，凭前端回传的目标归属校验后写入清理后的正文。
  ...
  function convertPlanToTask(projectId, planId) {
    ...
    // 任务名 = 方案标题（截断至任务名上限 60 字），描述 = 轻量摘要引用（防方案全量复制导致
    // 数据冗余与旧快照漂移；方案全文通过任务详情的方案关联查看）。摘要 = 固定前缀 + 正文剥标签纯文本前 200 字（供列表预览）
 // planIds=[planId] 让任务创建时即写 task_plans 双向关联
    const excerpt = htmlToPlain(cur.content).slice(0, 200);
    const { task, planIds } = buildTaskObject(projectId, {
      name: cur.title.length > 60 ? `${cur.title.slice(0, 60)}…` : cur.title,
      description: excerpt
        ? `由方案《${cur.title}》转出，详情见关联方案。方案摘要：${excerpt}`
        : `由方案《${cur.title}》转出，详情见关联方案。`,
      priority: "P3",
      planIds: [planId],
    });
 ...
 // ===== 批量操作（范式对齐 tasks 模块）=====
  ...
  /**
   * 同一目标（方案/需求）下批量加评论（事务包裹：任一条失败整体回滚，对齐 createTasks 范式）
   * @param {string} projectId
   * @param {string} targetType plan / requirement
   * @param {string} targetId 目标 ID
   * @param {Array<{content: string, quoteText?, quoteAnchor?}>} items 最多 50 条
   * @returns {Array} 创建后的评论对象列表
   */
  function addComments(projectId, targetType, targetId, items) {
    if (!COMMENT_TARGET_TYPES.includes(targetType)) {
      throw new Error(`不支持的评论对象类型: ${targetType}（可选 plan / requirement）`);
    }
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多添加 50 条评论");
    for (const [i, it] of items.entries()) {
      if (!it || !String(it.content || "").trim()) throw new Error(`第 ${i + 1} 条评论内容不能为空`);
    }
    return db.transaction(() =>
      items.map((it, i) => {
        try {
          return addComment(projectId, targetType, targetId, it.content, it.quoteText, it.quoteAnchor);
        } catch (e) {
          throw new Error(`第 ${i + 1} 条评论：${e.message}`);
        }
      })
    )();
  }
  ...
  /**
   * 批量编辑评论（逐条独立语义，对齐 updateTasks 范式）
   * @param {Array<{id: string, content: string}>} items 最多 50 条
   * @returns {{success: Array<{id:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function updateComments(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多编辑 50 条评论");
    const success = [];
    const failed = [];
    items.forEach((it, i) => {
      const index = i + 1;
      if (!it || !it.id) {
        failed.push({ id: it?.id || null, index, error: "缺少评论 ID" });
        return;
      }
      try {
        updateComment(projectId, it.id, it.content);
  ...
  /**
   * 批量删除评论（逐条独立语义；划词引用正文清理仍由前端 applyQuoteAnchor(cleanup) 流程负责）
   * @param {string[]} commentIds 最多 50 个
   * @returns {{success: Array<{id:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function deleteComments(projectId, commentIds) {
    if (!Array.isArray(commentIds) || commentIds.length === 0) throw new Error("commentIds 不能为空");
    if (commentIds.length > 50) throw new Error("单次最多删除 50 条评论");
    const success = [];
    const failed = [];
    commentIds.forEach((id, i) => {
      const index = i + 1;
      if (!id) {
        failed.push({ id: null, index, error: "缺少评论 ID" });
        return;
      }
      try {
