...
export function createMessagesModule(ctx) {
  const { db, shortId, localToday, addDays, getMessageConfig, logAudit } = ctx;
  const listProjects = (...a) => ctx.listProjects(...a);
  const summarizeProject = (...a) => ctx.summarizeProject(...a);
 // ===== 消息中心）=====
 ...
 // synergy（协同通知）：预留骨架，当前不生成消息
  ...
  /**
 * 删除单条消息（补审：按消息所属项目留痕，project_id 为空归 NULL 不进项目）
   * @param {string} id
   * @returns {boolean}
   */
