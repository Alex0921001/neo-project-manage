...
export function createRequirementsModule(ctx) {
 ...
 // 需求排序：default=创建时间倒序；priority=优先级 P0→P5 升序（非法/空归最低档 6），同级按创建时间倒序
 ...
 // ===== 批量操作（范式对齐 tasks 模块）=====
  ...
  /**
   * 批量创建需求（事务包裹：任一条失败整体回滚，对齐 createTasks 范式）
   * @param {string} projectId
   * @param {Array<{name: string, description?, priority?, planIds?}>} items 最多 50 条
   * @returns {Array} 创建后的需求对象列表
   */
  function createRequirements(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多创建 50 个需求");
    // 名称整体预校验（提前失败，避免事务内第一条就抛错）
    for (const [i, it] of items.entries()) {
      if (!it || !String(it.name || "").trim()) throw new Error(`第 ${i + 1} 个需求缺少名称`);
    }
    return db.transaction(() =>
      items.map((it, i) => {
        try {
          return createRequirement(projectId, it);
        } catch (e) {
          throw new Error(`第 ${i + 1} 个需求：${e.message}`);
        }
      })
    )();
  }
  ...
  /**
   * 批量更新需求（逐条独立语义，对齐 updateTasks 范式）
   * 每条走 updateRequirement 完整校验（含「仅待处理可改」），单条失败不影响其他条
   * @returns {{success: Array<{id:string,name:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function updateRequirements(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多更新 50 个需求");
    const success = [];
    const failed = [];
    items.forEach((it, i) => {
      const index = i + 1;
      if (!it || !it.id) {
        failed.push({ id: it?.id || null, index, error: "缺少需求 ID" });
        return;
      }
      try {
        const updated = updateRequirement(projectId, it.id, it);
  ...
  /**
   * 批量流转需求状态（逐条独立语义；items[].status 为目标态）
   * @returns {{success: Array<{id:string,name:string,status:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function updateRequirementStatuses(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多更新 50 个需求状态");
    const success = [];
    const failed = [];
    items.forEach((it, i) => {
      const index = i + 1;
      ...
      try {
        const updated = updateRequirementStatus(projectId, it.id, it.status);
        success.push({ id: it.id, name: updated.name, status: updated.status });
      } catch (e) {
        failed.push({ id: it.id, index, error: e.message });
      }
    });
  ...
  /**
   * 批量删除需求（逐条独立语义；「已完成不可删」等限制逐条生效）
   * @returns {{success: Array<{id:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function deleteRequirements(projectId, ids) {
    if (!Array.isArray(ids) || ids.length === 0) throw new Error("requirementIds 不能为空");
    if (ids.length > 50) throw new Error("单次最多删除 50 个需求");
    const success = [];
    const failed = [];
    ids.forEach((id, i) => {
      const index = i + 1;
      if (!id) {
        failed.push({ id: null, index, error: "缺少需求 ID" });
        return;
      }
      try {
