 ...
 * 先删该项目旧条目（FTS5 删除受限，须按 rowid 逐行删），再全量插入；事务内调用
 ...
 // project：项目名 + 描述（全局搜索优化：项目名纳入索引；refId=项目 id，跳转=打开项目页）
 ...
 // comment：统一评论（需求/方案共用）：title=截断内容，body=全文；ref_id 存 target_type|target_id 供跳转定位
 ...
 // verification_item：验证项内容，ref_id=所属验证卡 id（快速搜索）
 ...
 // verification_category：分类名（分组管理）
 ...
 * project：项目名/描述 LIKE（全局搜索优化：项目名纳入索引）
   */
 ...
 // 统一评论：需求/方案共用；ref_id=target_type|target_id 供跳转定位
    if (want("comment")) {
      push(db.prepare(`
 ...
 // 验证分类（分组管理）：ref_id=分类名
  ...
  /**
 * 全类型全文检索）：项目/任务/批注/方案/需求/备注/临时任务 + 文件名统一搜索
