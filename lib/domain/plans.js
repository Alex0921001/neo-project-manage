...
export function createPlansModule(ctx) {
 ...
 // 关联校验前置：任何 ID 非法则在 INSERT 前抛错，避免产生幽灵方案
 ...
 // requirement_plans / task_plans 无强依赖须应用层清理（防残留）；plan_comments 由外键级联删除
 // 统一评论表无 SQL 外键（同列多引用），应用层级联清评论
    db.transaction(() => {
      db.prepare("DELETE FROM requirement_plans WHERE plan_id = ?").run(planId);
      db.prepare("DELETE FROM task_plans WHERE plan_id = ?").run(planId);
      db.prepare("DELETE FROM comments WHERE target_type = 'plan' AND target_id = ?").run(planId);
 // 验证卡以任务为主、方案为弱关联（plan_ids JSON）：仅解除关联，不删卡（修 no such column）
 ...
 // 条件：项目归属 + 标题模糊搜索 + 状态精确筛选（方案分页/筛选）+ 精确 id
 ...
 // 修复：评论一律用解析后的归属项目查（全局短前缀查询时入参 projectId 为 null 会查空）
 ...
 // ===== 批量操作（范式对齐 tasks 模块）=====
  ...
  /**
   * 批量创建方案（事务包裹：任一条失败整体回滚，对齐 createTasks 范式）
   * @param {string} projectId
   * @param {Array<{title: string, content?, requirementIds?, taskIds?}>} items 最多 50 条
   * @returns {Array} 创建后的方案对象列表
   */
  function createPlans(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多创建 50 个方案");
    for (const [i, it] of items.entries()) {
      if (!it || !String(it.title || "").trim()) throw new Error(`第 ${i + 1} 个方案缺少标题`);
    }
    return db.transaction(() =>
      items.map((it, i) => {
        try {
          return createPlan(projectId, it.title, it.content, it.requirementIds, it.taskIds);
        } catch (e) {
          throw new Error(`第 ${i + 1} 个方案：${e.message}`);
        }
      })
    )();
  }
  ...
  /**
   * 批量更新方案（逐条独立语义，对齐 updateTasks 范式）
   * 每条走 updatePlan 完整校验（标题/内容仅草稿/进行中可改、已转任务状态冻结），单条失败不影响其他条
   * @returns {{success: Array<{id:string,title:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function updatePlans(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多更新 50 个方案");
    const success = [];
    const failed = [];
    items.forEach((it, i) => {
      const index = i + 1;
      if (!it || !it.id) {
        failed.push({ id: it?.id || null, index, error: "缺少方案 ID" });
        return;
      }
      try {
        const updated = updatePlan(projectId, it.id, it);
        success.push({ id: it.id, title: updated.title });
      } catch (e) {
  ...
  /**
   * 批量删除方案（逐条独立语义；「仅草稿/已废弃可删」限制逐条生效）
   * @returns {{success: Array<{id:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function deletePlans(projectId, ids) {
    if (!Array.isArray(ids) || ids.length === 0) throw new Error("planIds 不能为空");
    if (ids.length > 50) throw new Error("单次最多删除 50 个方案");
    const success = [];
    const failed = [];
    ids.forEach((id, i) => {
      const index = i + 1;
      if (!id) {
        failed.push({ id: null, index, error: "缺少方案 ID" });
        return;
      }
      try {
        deletePlan(projectId, id);
        success.push({ id });
      } catch (e) {
