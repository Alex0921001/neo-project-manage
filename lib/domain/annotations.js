...
export function createAnnotationsModule(ctx) {
  ...
  const noteContentEmpty = (...a) => ctx.noteContentEmpty(...a);
  // ===== Annotations =====

 // 批注类型：note=备注（默认）/ decision=决策 / risk=风险 / milestone=节点
 ...
 //  规则：任务已完成，便利贴冻结（内容/类型/确认状态均不可改，删除不受限）
 ...
 // 审计：批注签名无 projectId，从所属任务反查项目归属（审计追踪）
 ...
 //  规则：任务已完成不允许挂载便利贴
  ...
  /**
 * 批量确认批注：解决任务完成前置校验（全部便利贴须已确认）逐个确认的痛点
