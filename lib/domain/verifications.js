...
export function createVerificationsModule(ctx) {
 ...
 // ===== 验证模块（重构：验证卡（名称/关联任务/备注）+ 卡内验证项清单）=====
 ...
 /** 全局按 ID/唯一短前缀查验证卡（无需 projectId）：对齐 get_plan / get_task 查询体系 */
  ...
  function verificationItemRowToObject(r) {
    ...
    if (r.evidence) {
      try {
        const v = JSON.parse(r.evidence);
        if (v && typeof v === "object") evidence = v;
      } catch { evidence = null; } // 坏 JSON 不抛出，按无证据展示
    }
    return {
      id: r.id,
  ...
  function createVerificationItem(projectId, verificationId, data = {}) {
    ...
    if (data.kind !== undefined) {
      ...
      if (!["human", "agent", "assertion"].includes(kind)) {
        throw new Error(`非法的执行方式: ${kind}（可选 human / agent / assertion）`);
      }
    }
    ...
    db.prepare("INSERT INTO verification_items (id, verification_id, category, content, note, kind, instruction, created_at) VALUES (?,?,?,?,?,?,?,?)")
      .run(item.id, verificationId, item.category, item.content, item.note, item.kind, item.instruction, item.createdAt);
    logAudit(projectId, "创建验证项", "verification_item", item.id, null,
      JSON.stringify({ verificationId, category: item.category, kind: item.kind, content: auditText(content) }));
  ...
  function updateVerificationItem(projectId, id, data = {}) {
    ...
    const instruction = data.instruction !== undefined ? String(data.instruction).trim() : row.instruction || "";
    let kind = row.kind || "human";
    if (data.kind !== undefined) {
      kind = String(data.kind);
      if (!["human", "agent", "assertion"].includes(kind)) {
        throw new Error(`非法的执行方式: ${kind}（可选 human / agent / assertion）`);
    ...
    db.prepare("UPDATE verification_items SET content = ?, note = ?, category = ?, kind = ?, instruction = ? WHERE id = ?").run(content, note, category, kind, instruction, id);
 ...
 // ===== 数据断言（P1）：instruction 存四元组 JSON，后端查库断言（方案 2997e698）=====

 // 断言目标表白名单（防任意查询）：target 按「表名:id」解析，表名必须在集合内
  const ASSERTION_TABLES = new Set([
    "projects", "tasks", "plans", "requirements", "comments", "verifications",
    "verification_items", "quick_tasks", "annotations", "notes", "files", "versions",
  ]);
 // 第一版操作符集合（第八节「不做」：无 DSL、无嵌套、无组合、无聚合）
  const ASSERTION_OPS = new Set([
    "not_null", "null", "eq", "neq", "gt", "gte", "lt", "lte", "contains",
  ]);
  /** 操作符中文标签（前端摘要同口径） */
  const ASSERTION_OP_LABELS = {
    not_null: "非空", null: "为空", eq: "等于", neq: "不等于",
    gt: "大于", gte: "≥", lt: "小于", lte: "≤", contains: "包含",
  };
 // 各表 JSON 列（取值后先解析再比较；解析失败按原字符串参与比较）
  const ASSERTION_JSON_COLUMNS = {
    projects: ["members", "session_ids"],
    tasks: ["assignees"],
    comments: ["quote_anchor", "quote_text"],
    verifications: ["task_ids", "plan_ids"],
    verification_items: ["evidence"],
    versions: ["extra_json"],
  };

  /**
   * 解析断言 instruction（四元组 JSON：{target:"表名:id", field, op, value}）
   * @returns {{ok: true, assertion: {table,rowId,field,op,value}} | {ok: false, reason: string}}
   *   ok=false = 结构不合法（invalid），不执行不报错
   */
  function parseAssertionInstruction(instruction) {
    let a;
    try {
      a = JSON.parse(instruction);
    } catch {
      return { ok: false, reason: "断言定义不是合法 JSON" };
    }
    if (!a || typeof a !== "object" || Array.isArray(a)) {
      return { ok: false, reason: "断言定义必须是对象" };
    }
    ...
    if (!target) return { ok: false, reason: "断言缺少 target（表名:id）" };
    const sep = target.indexOf(":");
    if (sep <= 0 || sep === target.length - 1) {
      return { ok: false, reason: `target 必须为「表名:id」格式，收到「${target}」` };
    }
    const table = target.slice(0, sep);
    const rowId = target.slice(sep + 1);
    if (!ASSERTION_TABLES.has(table)) {
      return { ok: false, reason: `目标表「${table}」不在断言白名单` };
    }
    if (!field) return { ok: false, reason: "断言缺少 field" };
    if (!ASSERTION_OPS.has(op)) return { ok: false, reason: `非法的断言操作符: ${op}` };
    return { ok: true, assertion: { table, rowId, field, op, value: a.value ?? null } };
  }

  /** JSON 列取解析后的值参与比较（普通列原样取，缺列/空值兑底 null） */
  function assertionCellValue(row, table, field) {
    const raw = row[field];
    const jsonCols = ASSERTION_JSON_COLUMNS[table] || [];
    if (jsonCols.includes(field) && typeof raw === "string") {
      try { return JSON.parse(raw); } catch { return raw; }
    }
    return raw === undefined || raw === null ? null : raw;
  }

  /** eq 的类型宽容比较：期望数值按 Number 比较对象（数组/对象按 JSON 序列化） */
  function assertionEq(actual, expected) {
    if (typeof expected === "number" || typeof expected === "boolean") {
      if (actual === null || actual === "") return false;
      const a = Number(actual);
      return Number.isFinite(a) && a === Number(expected);
    }
  ...
  /** 按操作符比较，返回 {pass, reason?}；reason 为失败原因（拒勾依据） */
  function assertionCompare(actual, op, expected) {
    const empty = actual === null || actual === "" || actual === undefined;
    switch (op) {
      case "not_null":
        return empty ? { pass: false, reason: "字段为空" } : { pass: true };
      case "null":
        return empty ? { pass: true } : { pass: false, reason: "字段非空" };
      case "eq":
        return assertionEq(actual, expected)
          ? { pass: true }
          : { pass: false, reason: `期望 ${JSON.stringify(expected)}，实际 ${JSON.stringify(actual)}` };
      case "neq":
        return !assertionEq(actual, expected)
          ? { pass: true }
          : { pass: false, reason: `不期望相等，实际 ${JSON.stringify(actual)}` };
      case "contains":
        return String(actual ?? "").includes(String(expected ?? ""))
          ? { pass: true }
          : { pass: false, reason: `值 ${JSON.stringify(actual)} 不包含「${expected}」` };
      default: {
        // gt / gte / lt / lte：数值比较
        if (empty || !Number.isFinite(Number(actual))) {
          return { pass: false, reason: `字段值 ${JSON.stringify(actual)} 不是数值，无法数值比较` };
        }
        if (expected === null || expected === "" || !Number.isFinite(Number(expected))) {
          return { pass: false, reason: `期望值 ${JSON.stringify(expected)} 不是数值` };
        }
        const a = Number(actual);
        ...
        const okByOp =
          ...
          op === "gte" ? a >= e :
          op === "lt" ? a < e : a <= e;
        return okByOp ? { pass: true } : { pass: false, reason: `期望 ${ASSERTION_OP_LABELS[op]} ${e}，实际 ${a}` };
      }
    }
  }

  /**
   * 执行单条数据断言（安全边界：表名白名单 + 操作符集合，表名插值前已过白名单校验）
   * @param {{instruction?: string}} item 验证项行（kind=assertion）
   * @returns {{state: "pass"|"fail"|"invalid", reason?: string}}
   *   invalid = 结构不合法（JSON 解析失败/白名单外/缺字段/非法操作符），不执行不报错
   *   fail    = 结构合法但断言不通过（含原因），拒勾
   *   pass    = 断言通过，可回填证据自动勾选
   */
  function runAssertion(item) {
    const parsed = parseAssertionInstruction(item?.instruction || "");
    if (!parsed.ok) return { state: "invalid", reason: parsed.reason };
    const { table, rowId, field, op, value } = parsed.assertion;
    const row = db.prepare(`SELECT * FROM ${table} WHERE id = ?`).get(rowId);
    if (!row) return { state: "fail", reason: `目标 ${table}:${rowId} 不存在` };
    if (!(field in row)) return { state: "invalid", reason: `表 ${table} 无字段「${field}」` };
    const actual = assertionCellValue(row, table, field);
    const cmp = assertionCompare(actual, op, value);
    return cmp.pass ? { state: "pass" } : { state: "fail", reason: cmp.reason || "断言不通过" };
  }

  /**
   * 批量执行断言验证项（kind=assertion）：逐条独立，对齐批量操作范式。
   * 通过 → 回填 evidence（runner:"assertion"）自动勾选；已勾选项幂等更新证据，审计记「验证证据更新」。
   * 失败/invalid → 返回原因，拒绝勾选（护栏语义与 agent 项一致）。
   * @param {string[]} ids 最多 50 个
   * @returns {{success: Array<{id:string, updated:boolean, summary:string}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function verifyAssertionItems(projectId, ids) {
    if (!Array.isArray(ids) || ids.length === 0) throw new Error("ids 不能为空");
    if (ids.length > 50) throw new Error("单次最多执行 50 个断言项");
    const success = [];
    const failed = [];
    ids.forEach((id, i) => {
      const index = i + 1;
      ...
      try {
        ...
        if (!row) throw new Error(`验证项 ${id} 不存在`);
        getVerificationRowOrThrow(projectId, row.verification_id);
        if ((row.kind || "human") !== "assertion") {
          throw new Error(`验证项 ${id} 不是 assertion 类型（当前 ${row.kind || "human"}），断言执行仅支持 assertion 项`);
        }
        const res = runAssertion(row);
        if (res.state === "invalid") throw new Error(`断言定义无效：${res.reason}`);
        if (res.state === "fail") throw new Error(`断言未通过：${res.reason}`);
        const assertion = parseAssertionInstruction(row.instruction).assertion;
        const now = new Date().toISOString();
        const summary = `断言通过：${assertion.field} ${ASSERTION_OP_LABELS[assertion.op] || assertion.op}`;
        const evidence = JSON.stringify({
          runner: "assertion",
          runAt: now,
          summary,
          detail: `target=${assertion.table}:${assertion.rowId} · op=${assertion.op}${
            assertion.value !== null && assertion.value !== undefined ? ` · value=${JSON.stringify(assertion.value)}` : ""
          }`,
        });
        db.prepare("UPDATE verification_items SET status = 1, checked_at = ?, checked_by = ?, evidence = ? WHERE id = ?")
          .run(now, "assertion", evidence, id);
        const wasDone = !!row.status;
        logAudit(projectId, wasDone ? "验证证据更新" : "验证通过", "verification_item", id,
          JSON.stringify({ status: row.status }),
          JSON.stringify({ status: 1, evidence: { runner: "assertion", runAt: now, summary: auditText(summary) } }));
        success.push({ id, updated: wasDone, summary });
      } catch (e) {
        failed.push({ id: id || null, index, error: e.message });
      }
    });
  ...
  /**
 * 证据回填（Agent 化）：Agent 执行验证指令后回填证据并自动勾选。
   * 仅接受 kind=agent 的验证项（human 项走 toggle；assertion 项待断言引擎）。
   * 批量逐条独立：单条失败不影响其他；已勾选项幂等更新证据（重跑场景），审计记「验证证据更新」。
   * @param {Array<{id: string, evidence: {runner: string, summary: string, detail?: string, runAt?: string}}>} items 最多 50 条
   * @returns {{success: Array<{id:string, updated:boolean}>, failed: Array<{id:string|null,index:number,error:string}>}}
   */
  function reportVerificationResult(projectId, items) {
    if (!Array.isArray(items) || items.length === 0) throw new Error("items 不能为空");
    if (items.length > 50) throw new Error("单次最多回填 50 个验证项");
    const success = [];
    const failed = [];
    items.forEach((it, i) => {
      const index = i + 1;
      ...
      try {
        ...
        const runner = String(ev.runner || "").trim();
        const summary = String(ev.summary || "").trim();
        if (!runner) throw new Error("evidence.runner 不能为空（执行方标识，如 agent:common）");
        if (!summary) throw new Error("evidence.summary 不能为空（一句话结论）");
        const row = db.prepare("SELECT * FROM verification_items WHERE id = ?").get(it.id);
        if (!row) throw new Error(`验证项 ${it.id} 不存在`);
        getVerificationRowOrThrow(projectId, row.verification_id);
        if ((row.kind || "human") !== "agent") {
          throw new Error(`验证项 ${it.id} 不是 agent 类型（当前 ${row.kind || "human"}），证据回填仅支持 agent 项`);
        }
        const now = new Date().toISOString();
        const evidence = JSON.stringify({
          runner,
          runAt: ev.runAt ? String(ev.runAt) : now,
          summary,
          ...(ev.detail ? { detail: String(ev.detail) } : {}),
        });
        db.prepare("UPDATE verification_items SET status = 1, checked_at = ?, checked_by = ?, evidence = ? WHERE id = ?")
          .run(now, runner, evidence, it.id);
        const wasDone = !!row.status;
        logAudit(projectId, wasDone ? "验证证据更新" : "验证通过", "verification_item", it.id,
          JSON.stringify({ status: row.status }),
          JSON.stringify({ status: 1, evidence: { runner, runAt: now, summary: auditText(summary) } }));
        success.push({ id: it.id, updated: wasDone });
      } catch (e) {
        failed.push({ id: it?.id || null, index, error: e.message });
      }
    });
    return { success, failed };
  }
