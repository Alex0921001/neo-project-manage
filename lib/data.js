...
export function createDataAccess(dataDir) {
  ...
  function addComment(projectId, targetType, targetId, content, quote = null) {
    ...
    db.prepare(`
      INSERT INTO comments (id, project_id, target_type, target_id, content, author, created_at, quote_text)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(comment.id, projectId, targetType, targetId, comment.content, null, comment.createdAt, comment.quoteText);
