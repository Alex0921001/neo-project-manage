...
export function createDataAccess(dataDir) {
  ...
  function summarizeProject(projectId) {
    ...
    // 7. risk 类型批注（3.2）：每条 risk 批注生成一条风险（全部列出，确认态决定等级）
    // 设计决定：risk 批注本身就是风险登记，确认后视为已知风险仍保留；未确认 medium / 已确认 high
    for (const t of all) {
      for (const a of t.annotations || []) {
        ...
        risks.push({
          ...
          desc: `风险批注：${truncateText(htmlToPlain(a.content), 50)}（${truncateText(t.name, 20)}）`,
          tasks: [{ id: t.id, name: t.name, annotationId: a.id }],
