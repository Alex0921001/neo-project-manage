...
export function createDataAccess(dataDir) {
  ...
  function insertFtsEntries(projectId) {
    ...
    // verification：验证项内容（V2.6）
    for (const v of db.prepare("SELECT id, content FROM verifications WHERE project_id = ?").all(projectId)) {
      ins.run(`verification|${v.id}`, projectId, "verification", v.id,
        truncateText(htmlToPlain(v.content), 80), htmlToPlain(v.content));
    }
  }
  ...
  function likeSearch(kw, { projectId, type, limit }) {
    ...
    if (want("verification")) {
      push(db.prepare(`
        SELECT v.id AS ref_id, v.content AS title, p.id AS project_id, p.name AS project_name
        FROM verifications v JOIN projects p ON p.id = v.project_id
        WHERE v.content LIKE ? ESCAPE '\\' ${projCond}
      `).all(like, ...projParams), "verification");
    }
  ...
  function deletePlan(projectId, planId) {
    ...
    db.transaction(() => {
      ...
      db.prepare("DELETE FROM verifications WHERE target_type = 'plan' AND target_id = ?").run(planId);
  ...
  // ===== 验证模块（V2.6，verifications 表：一句话检查项 + 两态勾选 + 全量审计）=====

  const VERIFY_TEMPLATES = {
    standard: { name: "标准三件套", categories: ["功能验证", "边界与异常", "回归验证"] },
    ui: { name: "UI 走查", categories: ["视觉还原", "交互细节", "多分辨率表现"] },
    compat: { name: "兼容性", categories: ["多端表现", "数据迁移影响"] },
  };
  ...
  function listVerifications(projectId, opts = {}) {
    ...
    if (opts.targetType && opts.targetId) {
      conds.push("target_type = ?", "target_id = ?");
      args.push(opts.targetType, opts.targetId);
    }
    if (opts.category) { conds.push("category = ?"); args.push(opts.category); }
    if (opts.status === true || opts.status === "1") { conds.push("status = 1"); }
    if (opts.status === false || opts.status === "0") { conds.push("status = 0"); }
    const rows = db.prepare(
      `SELECT * FROM verifications WHERE ${conds.join(" AND ")} ORDER BY created_at ASC, id`
    ).all(...args);
    return { total: rows.length, items: rows.map(verificationRowToObject) };
  }

  function createVerification(projectId, data = {}) {
    const content = String(data.content || "").trim();
    if (!content) throw new Error("验证内容不能为空");
    const targetType = data.targetType || null;
    const targetId = data.targetId || null;
    if (targetType && !targetId) throw new Error("指定了对象类型但缺少对象 ID");
    if (targetType && !["plan", "requirement"].includes(targetType)) {
      throw new Error(`不支持的验证对象类型: ${targetType}`);
    }
    if (targetType) {
      // 归属校验（支持短前缀）
      if (targetType === "plan") getPlanRowOrThrow(projectId, targetId);
      else {
        const req = resolveRowById("requirements", "id", "project_id", projectId, targetId, "需求", (r) => r.name);
        if (!req) throw new Error(`需求 ${targetId} 不存在`);
        data.targetId = req.id;
      }
    }
    const item = {
    ...
    db.prepare(`
      INSERT INTO verifications (id, project_id, target_type, target_id, category, content, note, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(item.id, projectId, item.targetType, item.targetId, item.category, item.content, item.note, item.createdAt);
    logAudit(projectId, "创建验证项", "verification", item.id, null,
      JSON.stringify({ category: item.category, content: auditText(content) }));
    markFtsDirty(projectId);
    return verificationRowToObject(db.prepare("SELECT * FROM verifications WHERE id = ?").get(item.id));
  }
  ...
  function updateVerification(projectId, id, data = {}) {
    ...
    db.prepare("UPDATE verifications SET content = ?, note = ?, category = ?, updated_at = ? WHERE id = ?")
      .run(content, note, category, new Date().toISOString(), id);
    logAudit(projectId, "编辑验证项", "verification", id,
      JSON.stringify({ content: auditText(row.content) }),
      JSON.stringify({ content: auditText(content) }));
    markFtsDirty(projectId);
    return verificationRowToObject(db.prepare("SELECT * FROM verifications WHERE id = ?").get(id));
  }

  /** 勾选/退回（独立端点语义：状态变更事件，写 checked_at/by 并审计） */
  function toggleVerification(projectId, id) {
    const row = db.prepare("SELECT * FROM verifications WHERE id = ? AND project_id = ?").get(id, projectId);
    if (!row) throw new Error(`验证项 ${id} 不存在`);
    const now = new Date().toISOString();
    const next = row.status ? 0 : 1;
    db.prepare("UPDATE verifications SET status = ?, checked_at = ?, checked_by = ?, updated_at = ? WHERE id = ?")
      .run(next, next ? now : null, next ? "owner" : null, now, id);
    logAudit(projectId, next ? "验证通过" : "验证退回", "verification", id,
  ...
  function deleteVerification(projectId, id) {
    ...
    logAudit(projectId, "删除验证项", "verification", id,
  ...
  /** 按模板生成空条目（已填类别，内容待补） */
  function generateFromTemplate(projectId, data = {}) {
    const tpl = VERIFY_TEMPLATES[data.templateKey];
    if (!tpl) throw new Error(`未知模板: ${data.templateKey}（可选 ${Object.keys(VERIFY_TEMPLATES).join(" / ")}）`);
    const targetType = data.targetType || null;
    const targetId = data.targetId || null;
    const created = tpl.categories.map((cat) =>
      createVerification(projectId, { targetType, targetId, category: cat, content: `${cat}：` })
    );
    logAudit(projectId, "模板生成验证项", "verification", created[0]?.id || null,
      null, JSON.stringify({ template: tpl.name, count: created.length }));
    return created;
  }

  /** 看板聚合：总进度 + 各对象进度 + 通用组 */
  function verificationSummary(projectId) {
    const rows = db.prepare("SELECT * FROM verifications WHERE project_id = ?").all(projectId);
    const items = rows.map(verificationRowToObject);
    const total = items.length;
    const done = items.filter((i) => i.status).length;
    // 对象分组（需求/方案），通用（无 target）独立组
    const groups = new Map();
    for (const it of items) {
      const key = it.targetType && it.targetId ? `${it.targetType}|${it.targetId}` : "__general__";
    ...
    for (const [key, list] of groups) {
      const card = {
        ...
        failNotes: list.filter((i) => !i.status && i.note).length,
        remainingTitles: list.filter((i) => !i.status).slice(0, 2).map((i) => i.content),
        items: list,
      };
      if (key === "__general__") {
        card.general = true;
        ...
        card.targetId = tid;
        if (tt === "plan") {
          const p = db.prepare("SELECT title, status FROM plans WHERE id = ?").get(tid);
          card.title = p?.title || "（方案已删除）";
        } else {
          const r = db.prepare("SELECT name FROM requirements WHERE id = ?").get(tid);
          card.title = r?.name || "（需求已删除）";
        }
      }
      cards.push(card);
    }
    cards.sort((a, b) => (a.general === b.general ? b.done / (b.total || 1) - a.done / (a.total || 1) : a.general ? 1 : -1));
    return { total, done, cards };
  }
   ...
   */
  function applyQuoteAnchor(projectId, commentId, content) {
    const row = db.prepare("SELECT * FROM comments WHERE id = ? AND project_id = ?").get(commentId, projectId);
    if (!row) throw new Error(`评论 ${commentId} 不存在`);
    const html = String(content ?? "");
    if (!row.target_type || !row.target_id) throw new Error("该评论未挂载对象，无法写入引用标注");
    if (row.target_type === "plan") {
      const cur = getPlanRowOrThrow(projectId, row.target_id);
      db.prepare("UPDATE plans SET content = ?, updated_at = ? WHERE id = ? AND project_id = ?")
        .run(html, new Date().toISOString(), cur.id, projectId);
    } else {
      const cur = db.prepare("SELECT id FROM requirements WHERE id = ? AND project_id = ?").get(row.target_id, projectId);
      if (!cur) throw new Error(`需求 ${row.target_id} 不存在`);
      db.prepare("UPDATE requirements SET description = ? WHERE id = ? AND project_id = ?")
        .run(html, cur.id, projectId);
    }
    logAudit(projectId, "评论引用标注", "comment", commentId,
      null, JSON.stringify({ targetType: row.target_type, targetId: row.target_id }));
    markFtsDirty(projectId);
    return true;
  }
  ...
  function deleteRequirement(projectId, id) {
    ...
    const run = db.transaction(() => {
      ...
      db.prepare("DELETE FROM verifications WHERE target_type = 'requirement' AND target_id = ?").run(id);
