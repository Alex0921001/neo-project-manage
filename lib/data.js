...
export function createDataAccess(dataDir) {
  ...
  function getProjectFull(id) {
    const row = db.prepare(`
      SELECT id, name, description, members, plan_start, plan_end, status, project_set_id, session_ids, archived, archived_at, created_at
  ...
  function listProjects(projectSetId, keyword) {
    ...
    const rows = db.prepare(`
      SELECT id, name, description, members, plan_start, plan_end, status, project_set_id, archived, archived_at, created_at
      FROM projects
      ${whereSql}
  ...
  function updateProject(id, data) {
    ...
    const allowed = ["name", "description", "members", "planStart", "planEnd", "status", "projectSetId", "archived"];
    ...
    const map = {
      ...
      projectSetId: "project_set_id", archived: "archived", archivedAt: "archived_at",
    };
    const parts = [];
    const params = [];
    for (const [k, v] of Object.entries(sets)) {
      parts.push(`${map[k]} = ?`);
      // archived 是 0/1（0 不能转 null，否则撞 NOT NULL）；其余空值统一落 null
      params.push(k === "members" ? JSON.stringify(v) : (k === "archived" ? v : (v || null)));
    }
