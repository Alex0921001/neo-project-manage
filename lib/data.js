...
export function createDataAccess(dataDir) {
  ...
  function deleteFolder(projectId, folderId) {
    ...
    const run = db.transaction(() => {
      ...
      // 先统计再删除这些文件夹下的文件登记（磁盘文件不动）
      const fileCount = db.prepare(
        `SELECT COUNT(*) c FROM files WHERE project_id = ? AND folder_id IN (${placeholders})`
      ).get(projectId, ...ids).c;
      ...
      db.prepare(`DELETE FROM file_folders WHERE project_id = ? AND id IN (${placeholders})`)
        .run(projectId, ...ids);
      return { folderCount: ids.length, fileCount };
    });
    const { folderCount, fileCount } = run();
    logAudit(projectId, "删除文件夹", "folder", folderId,
      JSON.stringify({ name: cur.name, deletedFolders: folderCount, deletedFiles: fileCount }), null);
    return { id: folderId, deletedFolders: folderCount, deletedFiles: fileCount };
  }
