...
import { createDb, shortId, tx, SCHEMA_VERSION } from "./db.js";
...
export function createDataAccess(dataDir) {
  ...
  /**
   * 导出全部数据（项目集 + 完整项目：任务树/文件/备注/批注），用于备份
   */
  function exportAll() {
    const sets = db
      .prepare("SELECT id, name, created_at FROM project_sets ORDER BY name")
      .all()
      .map((s) => ({ id: s.id, name: s.name, createdAt: s.created_at }));
    const projRows = db.prepare("SELECT id FROM projects ORDER BY created_at DESC").all();
    const projects = projRows.map((r) => getProjectFull(r.id)).filter(Boolean);
    return {
