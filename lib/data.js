...
export function createDataAccess(dataDir) {
  ...
  function buildTaskObject(projectId, data) {
    ...
    // V2.2 R14：关联方案（白名单校验：必须属于本项目，防跨项目/脏数据）
  ...
  function updateTask(projectId, taskId, data) {
    ...
    // V2.2 R14：关联方案全量替换（白名单校验：必须属于本项目）
  ...
  function generateReport(projectId, rangeOpts = {}) {
    ...
    const ownerText = (t) => (t.assignees?.length ? `　负责人：${t.assignees.join("、")}` : "");
    const dueText = (t) => (t.endDate ? `　截止：${t.endDate}` : "");
    ...
    if (doneInRange.length === 0) {
      ...
      for (const t of doneInRange) {
        ...
        L.push(`- [x] ${t.name}${ownerText(t)}（完成于 ${at}）`);
      }
    }
    L.push("");
    ...
    if (pending.length === 0) {
      ...
      L.push("");
      for (const t of pending) L.push(`- [ ] ${t.name}${ownerText(t)}${dueText(t)}`);
    }
    L.push("");
    ...
    if (risks.length === 0) {
      ...
      for (const r of risks) L.push(`- ${icon[r.level] || "•"} [${r.level}] ${r.desc}`);
    }
    L.push("");
