...
export function createDataAccess(dataDir) {
  ...
  function listQuickTasks() {
    const rows = db.prepare(
      "SELECT * FROM quick_tasks WHERE status != 'archived' ORDER BY created_at DESC"
    ).all();
  ...
  /**
   * 删除：active 草稿可直接删；archived 在归档弹窗删；done/converted 不可直接删（走归档）
   */
  function deleteQuickTask(id) {
    const cur = getQuickTaskRow(id);
    if (cur.status === "done" || cur.status === "converted") {
      throw new Error("已完成/已转化任务不可直接删除，请先归档");
    }
