...
// 项目 session_ids 同为 JSON 字符串数组，复用同一解析逻辑（语义别名）
const parseSessionIds = parseMembers;
// 任务优先级等级：P0 最急 → P5 最缓，字符串序即等级序（任务等级）
...
export function createDataAccess(dataDir) {
  ...
  /**
 * 数据自愈：清理无外键级联的悬空引用（幂等、安全，数据访问层每次初始化执行，开销可忽略）
   * - plans.task_id 指向已删任务 → 置空（删除任务后方案可再次转任务）
   * - requirement_plans 指向已删方案/需求 → 删除（删除方案/需求后无关联残留）
 * - task_plans 指向已删任务/方案 → 删除；表虽声明了双外键级联，历史库/极端环境残留仍兜底清理）
   * 正常数据（引用仍存在）不受影响。
   */
 ...
 * 完成任务前置校验用，防「父完成 + 子未完成」矛盾树（与前端 TaskTab 同款逻辑）
   * @param {string} taskId
 ...
 // =====  批2拆分：模块组装（顺序 assign；跨模块依赖走转发箭头，顺序仅影响可读性）=====
  ...
  const { getSetting, setSetting, getMessageConfig, updateMessageConfig, insertMessage, scanMessages, listMessages, markMessageRead, deleteMessage, getMessageUnreadCount, listMembers, normalizeMemberName, createMember, renameMember, deleteMember, allKnownNames, noteContentEmpty, createNote, updateNote, deleteNote, quickNow, quickRow, getQuickTaskRow, syncQuickTaskFts, removeQuickTaskFts, listQuickTasks, createQuickTask, updateQuickTask, deleteQuickTask, archiveQuickTask, archiveQuickTasks, listArchivedQuickTasks, deleteArchivedQuickTasks, convertQuickTask, createQuickTasks, updateQuickTasks, deleteQuickTasks, getProjectSessionIds, linkProjectSession, listProjectSessions, unlinkProjectSession, listCalendarTasks, matchesMagic, saveUploadedFile, healDanglingReferences, resolveRowById, markFtsDirty, insertFtsEntries, rebuildFtsIndex, ensureFtsReady, toFtsPhrase, ftsSearch, likeSearch, searchAll, ftsFullIndexed, indexedProjectCount, auditText, logAudit, listProjectSets, getProjectSet, createProjectSet, updateProjectSet, deleteProjectSet, reorderProjectSets, getProjectSetWithProjectCount, listProjectSetsWithCounts, getTaskOrThrow, taskRowToObject, validateTaskDates, normalizePriority, normalizeMilestone, validateTaskName, validateAssignees, getTaskById, listTasks, buildTaskObject, insertTaskWithRefs, createTask, updateTask, createTasks, updateTasks, deleteTask, reorderTasks, reorderSubtasks, createSubTask, updateSubTask, deleteSubTask, wouldCreateCycle, moveTask, normalizeAnnotationKind, getTaskAnnotations, getProjectAnnotations, createAnnotation, updateAnnotation, deleteAnnotation, createAnnotations, deleteAnnotations, confirmAnnotations, updateAnnotations, listProjects, getProject, createProject, updateProject, deleteProject, saveProjectSummary, getProjectSummaries, fileRowToObject, readFileMeta, normalizeDigest, addFile, getFile, listFiles, moveFile, deleteFile, getFilePath, folderNameError, assertFolderSameLevelName, getFolderOrThrow, wouldCreateFolderCycle, createFolder, updateFolder, deleteFolder, getFolder, listFolders, planRowToObject, getPlanRowOrThrow, replacePlanRequirements, replacePlanTasks, createPlan, updatePlan, deletePlan, listPlans, getPlan, addPlanComment, deletePlanComment, createPlans, updatePlans, deletePlans, saveVersion, ensureBaselineVersion, deleteVersionsFor, listVersions, restoreVersion, setVersionLabel, verificationRowToObject, verificationWithMeta, listVerifications, getVerificationRowOrThrow, getVerificationGlobal, normalizeVerificationTaskIds, normalizeVerificationPlanIds, createVerification, updateVerification, deleteVerification, verificationItemRowToObject, listVerificationItems, createVerificationItem, updateVerificationItem, toggleVerificationItem, reportVerificationResult, deleteVerificationItem, runAssertion, verifyAssertionItems, createVerifications, createVerificationItems, updateVerificationItems, toggleVerificationItems, deleteVerificationItems, deleteVerifications, seedVerificationCategories, listVerificationCategories, createVerificationCategory, renameVerificationCategory, deleteVerificationCategory, clearVerificationItems, commentRowToObject, getComments, listAllComments, addComment, updateComment, deleteComment, applyQuoteAnchor, convertPlanToTask, addComments, updateComments, deleteComments, requirementRowToObject, getRequirement, replaceRequirementPlans, createRequirement, updateRequirement, updateRequirementStatus, unlinkPlanFromVerifications, deleteRequirement, listRequirements, linkRequirementPlans, unlinkRequirementPlans, createRequirements, updateRequirements, updateRequirementStatuses, deleteRequirements, getRiskConfig, updateRiskConfig, flattenTaskTree, summarizeProject, resolveReportRange, generateReport, timeToMillis, timelineTitle, summaryTitle, collectDecisions, collectTimeline, collectRequirements, collectPlans, askProject, listAuditLogs } = ctx;
