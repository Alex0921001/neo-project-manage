...
export function createDataAccess(dataDir) {
  ...
  function summarizeProject(projectId) {
    ...
    if ((!R || R.delayed.enabled) && delayed.length > 0) {
      risks.push({ level: R?.delayed.level || "high", desc: `${delayed.length} 个任务已延期（最长延期 ${delayed[0].days} 天）`, tasks: delayed.map((t) => ({ id: t.id, name: t.name })) });
    }
    // 2. 逼近截止（level/days 可配）：任务 endDate 在 days 天内 且 未完成
    if ((!R || R.nearDeadline.enabled) && nearDeadline.length > 0) {
      risks.push({ level: R?.nearDeadline.level || "medium", desc: `${nearDeadline.length} 个任务将在 ${nearDays} 天内到期`, tasks: nearDeadline.map((t) => ({ id: t.id, name: t.name })) });
    }
    ...
    if ((!R || R.projectOverdue.enabled) && overdueProject) {
      risks.push({ level: R?.projectOverdue.level || "high", desc: `项目已超过计划结束日期（${project.planEnd}）`, tasks: [] });
    }
    ...
    if ((!R || R.noDateTasks.enabled) && total >= (R?.noDateTasks.minTotal ?? 3) && noDateCount / total >= (R?.noDateTasks.ratio ?? 0.6)) {
      risks.push({ level: R?.noDateTasks.level || "low", desc: `${noDateCount}/${total} 个任务缺少起止日期`, tasks: noDateTasks.map((t) => ({ id: t.id, name: t.name })) });
    }
    // 6. 项目停滞（level 可配）：状态「进行中」但 0 个未完成任务
    if ((!R || R.projectStagnant.enabled) && project.status === "进行中" && pendingTasks.length === 0) {
      risks.push({ level: R?.projectStagnant.level || "medium", desc: "项目状态为「进行中」但没有未完成任务", tasks: [] });
    }
