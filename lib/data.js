...
export function createDataAccess(dataDir) {
  ...
  function getProjectFull(id) {
    const row = db.prepare(`
      SELECT id, name, description, members, plan_start, plan_end, status, project_set_id, session_ids, created_at
  ...
  function summarizeProject(projectId) {
    ...
    const delayed = pendingTasks
      ...
      .map((t) => ({ task: truncateText(t.name), days: diffDays(today, t.endDate) }))
    ...
    const noDateCount = all.filter((t) => !t.startDate && !t.endDate).length;

    // 1. 延期 high：任务 endDate < 今天 且 未完成
    if (delayed.length > 0) {
      risks.push({ level: "high", desc: `${delayed.length} 个任务已延期（最长延期 ${delayed[0].days} 天）` });
    }
    // 2. 逼近截止 medium：任务 endDate 在 3 天内 且 未完成
    if (nearDeadline.length > 0) {
      risks.push({ level: "medium", desc: `${nearDeadline.length} 个任务将在 3 天内到期` });
    }
    // 3. 批注积压 medium：待确认批注 ≥ 3
    if (pendingAnnotations.length >= 3) {
      risks.push({ level: "medium", desc: `${pendingAnnotations.length} 条待确认批注待处理` });
    }
    // 4. 项目逾期 high：项目 planEnd < 今天 且 未完成
    if (overdueProject) {
      risks.push({ level: "high", desc: `项目已超过计划结束日期（${project.planEnd}）` });
    }
    // 5. 任务无日期 low：任务数 ≥ 3 且无日期任务 ≥ 60%
    if (total >= 3 && noDateCount / total >= 0.6) {
      risks.push({ level: "low", desc: `${noDateCount}/${total} 个任务缺少起止日期` });
    }
    // 6. 项目停滞 medium：状态「进行中」但 0 个未完成任务
    if (project.status === "进行中" && pendingTasks.length === 0) {
      risks.push({ level: "medium", desc: "项目状态为「进行中」但没有未完成任务" });
    }
    ...
    return {
      ...
      project: { name: project.name, status: computeStatus(project), progress },
      summary,
      completed,
      pending,
      delayed,
