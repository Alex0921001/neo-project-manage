...
export function createDataAccess(dataDir) {
  ...
  function buildTaskTree(flatTasks, fileRefsMap, annotationsMap) {
    ...
    for (const t of flatTasks) {
      const { start_date, end_date, assignees, is_milestone, ...rest } = t;
      byId.set(t.id, {
        ...rest,
  ...
  function getProjectTasks(projectId) {
    return db.prepare(`
      SELECT id, project_id, parent_task_id, index_num, name, description, done, assignees, start_date, end_date, priority, is_milestone, created_at
      FROM tasks
      WHERE project_id = ?
    `).all(projectId);
  }
  ...
  function getProjectStats(projectId) {
    ...
    return { taskCount, incompleteTaskCount, fileCount, noteCount, planCount };
  }
  ...
  function listTasks(projectId, filters = {}) {
    ...
    const rows = db.prepare(`
      SELECT ${distinct}t.id, t.project_id, t.parent_task_id, t.index_num, t.name, t.description, t.done, t.assignees, t.start_date, t.end_date, t.priority, t.is_milestone, t.created_at
      FROM tasks t
  ...
  function createTask(projectId, data) {
    ...
    db.transaction(() => {
      insertTask.run(task.id, task.project_id, task.parent_task_id, task.index_num, task.name, task.description, task.assignees, task.start_date, task.end_date, task.priority, task.is_milestone, task.created_at);
  ...
  /**
   * 删除文件夹（内容提升语义，不级联删除、不丢结构）：
   * 其下直接子文件与直接子文件夹整体提升到被删夹的父级，后代结构保持相对关系。
   */
  function deleteFolder(projectId, folderId) {
    const cur = getFolderOrThrow(projectId, folderId);
    const parentId = cur.parent_id || null;
    db.transaction(() => {
      db.prepare("UPDATE files SET folder_id = ? WHERE project_id = ? AND folder_id = ?").run(parentId, projectId, folderId);
      db.prepare("UPDATE file_folders SET parent_id = ? WHERE project_id = ? AND parent_id = ?").run(parentId, projectId, folderId);
      db.prepare("DELETE FROM file_folders WHERE id = ? AND project_id = ?").run(folderId, projectId);
    })();
    logAudit(projectId, "删除文件夹", "folder", folderId,
      JSON.stringify({ name: cur.name, parentId: cur.parent_id || null, promoted: true }), null);
    return true;
  }
  ...
  function createPlan(projectId, title, content = "") {
  ...
  function deletePlan(projectId, planId) {
    ...
    if (row.status !== "草稿" && row.status !== "已废弃") {
      throw new Error("仅「草稿 / 已废弃」状态的方案可删除");
    }
    // plan_comments 由外键级联删除
  ...
  function listPlans(projectId, opts = {}) {
    const { limit, offset, keyword, status } = opts;
    // 条件：项目归属 + 标题模糊搜索 + 状态精确筛选（V2.1 方案分页/筛选）
    ...
    let sql = `
      SELECT p.*, (SELECT COUNT(*) FROM plan_comments c WHERE c.plan_id = p.id) AS comment_count,
             t.name AS task_name
  ...
  function getPlan(projectId, planId) {
    ...
    const comments = db.prepare(
      "SELECT id, content, created_at AS createdAt FROM plan_comments WHERE plan_id = ? ORDER BY created_at ASC, id"
    ).all(planId);
  ...
  function addPlanComment(projectId, planId, content) {
    ...
    const comment = { id: shortId(), plan_id: planId, content: t, created_at: new Date().toISOString() };
    db.prepare("INSERT INTO plan_comments (id, plan_id, content, created_at) VALUES (?, ?, ?, ?)")
      .run(comment.id, comment.plan_id, comment.content, comment.created_at);
    logAudit(projectId, "方案评论", "plan_comment", comment.id, null,
      JSON.stringify({ planId, content: auditText(t) }));
    return { id: comment.id, content: t, createdAt: comment.created_at };
  }

  function deletePlanComment(projectId, planId, commentId) {
    getPlanRowOrThrow(projectId, planId);
    const row = db.prepare("SELECT content FROM plan_comments WHERE id = ? AND plan_id = ?").get(commentId, planId);
    const result = db.prepare("DELETE FROM plan_comments WHERE id = ? AND plan_id = ?").run(commentId, planId);
    if (result.changes === 0) throw new Error(`评论 ${commentId} 不存在`);
    if (row) logAudit(projectId, "删除方案评论", "plan_comment", commentId,
  ...
  // 三态：待处理 → 已完成 / 已取消（已完成/已取消冻结，不可再变更）
  ...
  function getRequirement(projectId, id) {
    const row = db.prepare("SELECT * FROM requirements WHERE id = ? AND project_id = ?").get(id, projectId);
    ...
    `).all(id);
    return { ...requirementRowToObject(row), planIds, plans };
  }
  ...
  function updateRequirementStatus(projectId, id, status) {
    ...
    if (cur.status !== "待处理") throw new Error("已完成/已取消的需求状态不可再变更");
  ...
  function deleteRequirement(projectId, id) {
    const row = db.prepare("SELECT name FROM requirements WHERE id = ? AND project_id = ?").get(id, projectId);
    if (!row) throw new Error(`需求 ${id} 不存在`);
    const run = db.transaction(() => {
  ...
  function listRequirements(projectId, opts = {}) {
    const { limit = 50, offset = 0, status, keyword } = opts;
    ...
    const rows = db.prepare(`SELECT * FROM requirements WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`)
  ...
  const DEFAULT_RISK_CONFIG = {
    ...
    riskAnnotation:    { enabled: true, unconfirmedLevel: "medium", confirmedLevel: "high" }, // 7.风险批注聚合
  };
  ...
  function summarizeProject(projectId) {
    ...
    // 7. risk 类型批注（聚合一条，等级可配）：明细在 tasks，点击气泡展示
    const riskAnnotations = [];
    for (const t of all) {
      for (const a of t.annotations || []) {
        if ((a.kind || "note") !== "risk") continue;
    ...
    if ((!R || R.riskAnnotation.enabled) && riskAnnotations.length > 0) {
      const confirmedLevel = R?.riskAnnotation.confirmedLevel || "high";
      const unconfirmedLevel = R?.riskAnnotation.unconfirmedLevel || "medium";
      risks.push({
        level: riskAnnotations.some((x) => x.confirmed) ? confirmedLevel : unconfirmedLevel,
        desc: `${riskAnnotations.length} 条风险批注`,
        tasks: riskAnnotations,
        kind: "risk",      // 来源标记：供前端/测试区分规则风险与批注风险
        category: "annotation",
      });
    }
