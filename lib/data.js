...
export function createDataAccess(dataDir) {
  ...
  function listVerifications(projectId, opts = {}) {
    ...
    if (opts.keyword) {
      conds.push("(name LIKE ? ESCAPE '\\' OR note LIKE ? ESCAPE '\\')");
      args.push(`%${escapeLike(opts.keyword)}%`, `%${escapeLike(opts.keyword)}%`);
    }
  ...
  function createVerification(projectId, data = {}) {
    ...
    const now = new Date().toISOString();
    const id = shortId();
    db.prepare("INSERT INTO verifications (id, project_id, name, note, task_ids, created_at, updated_at) VALUES (?,?,?,?,?,?,?)")
      .run(id, projectId, name, String(data.note || "").trim(), JSON.stringify(taskIds), now, now);
  ...
  function updateVerification(projectId, id, data = {}) {
    ...
    const taskIds = data.taskIds !== undefined ? normalizeVerificationTaskIds(projectId, data.taskIds) : verificationRowToObject(row).taskIds;
    db.prepare("UPDATE verifications SET name = ?, note = ?, task_ids = ?, updated_at = ? WHERE id = ?")
      .run(name, note, JSON.stringify(taskIds), new Date().toISOString(), id);
