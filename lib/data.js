...
export function createDataAccess(dataDir) {
  ...
  function createProject(data) {
    ...
    if (data.description && data.description.length > 200) throw new Error("项目描述最多200个字符");
    const project = {
      id: shortId(),
      name: data.name,
  ...
  function listTasks(projectId, filters = {}) {
    ...
    // assignee 精确匹配成员数组元素（json_each），空 assignee 时不做 join
    const joinJe = assignee ? ", json_each(p.members) je" : "";
    if (assignee) {
      where.push("je.value = ?");
    ...
    const rows = db.prepare(`
      SELECT ${distinct}t.id, t.project_id, t.parent_task_id, t.index_num, t.name, t.description, t.done, t.created_at
      FROM tasks t
  ...
  function createAnnotations(projectId, taskId, items) {
    ...
    return db.transaction(() => items.map((it) => createAnnotation(taskId, it)))();
  }
