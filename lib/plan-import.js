/**
 * 方案文件解析：txt / md / docx → { title, content(HTML) }
 *
 * 零新增依赖策略：
...
/**
 * docx → 纯文本（段落间换行），供 read_project_file 等文本提取场景复用
 *
 * 与 docxXmlToHtml 同一套 tar 解压 + <w:t> 提取链路，仅输出纯文本（去样式）。
 * @param {Buffer} bytes docx 文件内容
 * @returns {string} 纯文本，空串表示未提取到有效内容
 * @throws docx 无效 / 系统 tar 不可用
 */
export function docxToText(bytes) {
  const xml = readDocxXml(bytes);
