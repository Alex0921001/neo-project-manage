...
export const parameters = {
  ...
  properties: {
    ...
    kind: { type: "string", enum: ["human", "agent", "assertion"], description: "执行方式（可选，默认 human）：agent 项由 Agent 执行后回填证据；assertion 为数据断言（verify_assertion_items 执行）" },
    instruction: { type: "string", description: "Agent 验证指令（kind=agent 时提供）；kind=assertion 时为断言四元组 JSON，如 {\"target\":\"comment:36db7c28\",\"field\":\"quote_anchor\",\"op\":\"not_null\",\"value\":null}" },
  },
};
