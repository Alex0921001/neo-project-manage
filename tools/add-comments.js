...
export const description = "同一目标（方案/需求）下批量添加评论（最多 50 条，事务包裹：任一条校验失败整体回滚）。每项含 content（必填）、quoteText（可选纯文字引用，如原文摘录）。任何状态均可评论。注意：Agent 添加的引用为纯文字引用，正文无高亮标注（标注需在页面划词时生成），不可点击定位。";
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          quoteText: { type: "string", description: "引用的文字片段（可选，纯文字引用，正文无高亮标注不可定位）" },
        },
      },
    },
  },
